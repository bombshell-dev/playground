...
/** Read-only view of one pushed focus boundary. */
export interface FocusBoundaryInspection {
	root: Node;
	restore: Node | undefined;
}

/** Read-only focus state for diagnostics and semantic inspection. */
export interface FocusInspection {
	activeRoot: Node;
	focused: Node | undefined;
	stack: readonly FocusBoundaryInspection[];
}
...
/**
 * Inspect the active focus boundary without exposing the mutable internal
 * stack. This is intentionally read-only and contains node references rather
 * than application values.
 */
export function inspectFocus(node: Node): FocusInspection {
	const stack = focusStack(node);
	const activeRoot = focusRoot(node);
	return {
		activeRoot,
		focused: focusChain(activeRoot).find((candidate) => candidate.props.focused === true),
		stack: Object.freeze(
			stack.map((entry) => Object.freeze({ root: entry.node, restore: entry.restore })),
		),
	};
}
...
export function focusPush(node: Node, callback?: (value: unknown) => void): PopFocus {
	const stack = focusStack(node);
	const restore = focusChain(focusRoot(node)).find((n) => n.props.focused === true);
...
function restoreFocus(node: Node, restore: Node | undefined): void {
	...
	if (restore && chain.includes(restore)) {
		...
		if (first) {
			...
			const residual = focusChain(findRoot(node)).find((n) => n.props.focused === true);
			if (residual) {
