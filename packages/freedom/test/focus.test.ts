...
describe('Focus stack (§12)', () => {
	it('exposes a read-only inspection of the active root, focus, and restore target', () => {
		const root = createRoot();
		const before = root.node.createChild('before');
		focusable(before);
		const modal = root.node.createChild('modal');
		const inside = modal.createChild('inside');
		focusable(inside);
		useFocus(root.node);

		expect(inspectFocus(root.node)).toEqual({
		...
		const pushed = inspectFocus(root.node);
		expect(pushed.activeRoot).toBe(modal);
		expect(pushed.focused).toBe(inside);
		expect(pushed.stack).toEqual([{ root: modal, restore: before }]);
		expect(Object.isFrozen(pushed.stack)).toBe(true);
		expect(Object.isFrozen(pushed.stack[0])).toBe(true);
